...
def analyze_response(
    ...
    is_2xx = 200 <= (status_code or 0) < 300
    is_5xx = (status_code or 0) >= 500

    # Any server-side crash/error during security probes is suspicious.
    if is_5xx:
        return (
            "FAIL",
            f"Server returned {status_code} for security payload, indicating potential vulnerability or weak error handling.",
            "Harden input validation and exception handling; avoid internal server errors for untrusted input.",
        )
    ...
    if payload.owasp_category == "A07:BrokenAuth":
        ...
        if is_2xx and payload.attack_type in ("jwt_weak_secret", "jwt_expired", "empty_bearer", "null_auth"):
            return ("FAIL", f"Auth bypass payload accepted with status {status_code}.", "Verify JWT signature, expiration, and bearer token format strictly.")
    ...
    if payload.owasp_category == "A01:BrokenAccess":
        ...
        if is_2xx and payload.attack_type in ("idor_id_zero", "idor_id_neg", "idor_id_large", "idor_uuid_nil", "bola_admin_path"):
            return ("FAIL", f"Potential IDOR/BOLA: unauthorized object probe returned {status_code}.", "Enforce ownership checks on every object/resource lookup.")
