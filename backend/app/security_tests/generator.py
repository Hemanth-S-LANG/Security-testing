...
class GeneratedTestCase:
    ...
    execution_mode: str = "stateless"  # stateless | stateful
...
def _generate_for_endpoint(
    ...
    resolved_endpoint_path = _resolve_endpoint_path(endpoint)
    url_base = base_url.rstrip("/") + resolved_endpoint_path
    ...
    for payload in BROKEN_AUTH_PAYLOADS:
        if payload.inject_in == "header":
            ...
            cases.append(GeneratedTestCase(
                ...
                headers=headers,
                requires_auth=endpoint.requires_auth,
                execution_mode="stateful" if endpoint.requires_auth else "stateless",
            ))
    ...
    for payload in BROKEN_ACCESS_PAYLOADS:
        if payload.inject_in == "path":
            ...
            injected_path = resolved_endpoint_path
            if path_params:
                for pp in path_params:
                    injected_path = injected_path.replace(f"{{{pp.name}}}", str(payload.payload), 1)
            else:
                # Append to path if no params (probe extra segments)
                injected_path = resolved_endpoint_path.rstrip("/") + "/" + str(payload.payload)

            cases.append(GeneratedTestCase(
                endpoint_path=injected_path,
                http_method=endpoint.method,
                owasp_category=payload.owasp_category,
                attack_type=payload.attack_type,
                payload_used=str(payload.payload),
...
def _resolve_endpoint_path(endpoint: Endpoint) -> str:
    """
    Replace path placeholders with deterministic values so UI and logs show
    concrete request paths like /orders/101 instead of /orders/{id}.
    """
    path = endpoint.path
    for param in endpoint.params:
        if param.location != "path":
            continue
        replacement = _path_value_for_param(param.name, param.example)
        path = path.replace(f"{{{param.name}}}", replacement)
    return path


def _path_value_for_param(param_name: str, example: object | None) -> str:
